...
solve input = (part1, part2)
  where
    ls = rights . map (parseOnly lineSegmentParser) . T.lines $ T.pack input
    part1 = show . answer . map fullLine $ filter horzOrVertLine ls
    part2 = show . answer . map fullLine $ filter (liftA2 (||) horzOrVertLine fortyFiveDegreeLine) ls
