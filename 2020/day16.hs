...
main = do
  Right (rs, t, ts) <- parseOnly inputParser <$> T.readFile "day16.input"
  let (valid, invalid) = partition (isValidTicket rs) ts
  print . (++) "Part 1: " . show . sum $ allInvalids rs invalid
  print . (++) "Part 2: " . show . product . getFields t . getDepartureFields $ determineFields rs (t:valid)
