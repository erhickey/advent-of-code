...
main = do
  (polymer, rules) <- parse <$> readFile "day14.input"
  let steps = iterate (step rules) polymer
  print . (++) "Part 1: " . show . answer $ steps !! 10
  print . (++) "Part 2: " . show . answer $ steps !! 40
  print . (++) "Part 3: " . show . sum . M.elems . snd $ steps !! 40
